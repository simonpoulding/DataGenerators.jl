...
describe("Categorical Sampler") do
	...
	test("constructor") do
		...
		@check typeof(s.dist) == GodelTest.CategoricalDist
	end
	...
	test("setparams") do
		...
		@check typeof(s.dist) == GodelTest.CategoricalDist
		@check getparams(s) == [0.7, 0.1, 0.2]
	end

	test("invalid parameters are adjusted") do
		s = GodelTest.CategoricalSampler(4)
		setparams(s,[0.2, 0.1, 0.1, 0.1])
		@check getparams(s) == [0.4, 0.2, 0.2, 0.2]
		setparams(s,[0.5, 1.0, 0.5, 0.0])
		@check getparams(s) == [0.25, 0.5, 0.25, 0.0]
