type MCTSNode
	...
	function MCTSNode()
		new(0.0, 0.0, 0, :unknown, Nothing, 0.0, 0.0, Dict{Real,MCTSNode}())
	end
end

function printnode(node::MCTSNode, action::String="root", indent::Int=0)
	#DEBUG println(" "^(indent*2) * "$(action): totalreward: $(node.totalreward) maxreward: $(node.maxreward) visitcount: $(node.visitcount) cptype: $(node.cptype) datatype: $(node.datatype) lowerbound: $(node.lowerbound) upperbound: $(node.upperbound)")
